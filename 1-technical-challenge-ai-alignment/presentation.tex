...
\begin{frame}{Outer Alignment: Reward Misspecification}
    ...
    \begin{alertblock}{Real-World: Tall Falling Creatures}
        \begin{itemize}
            ...
            \item AI optimized for proxy target, not true goal!
        \end{itemize}
    \end{alertblock}
...
\begin{frame}{Inner Alignment: Goal Misgeneralization}
    ...
    About ensuring what the AI \textit{tries to pursue} (judged by action choices) matches the specified objective.
    ...
    \begin{alertblock}{Real-World: Grasping Illusion}
        \begin{itemize}
            \item Reward: Human evaluators approve grasping
            \item Learned: Position hand between ball and camera
            \item Optimized for \textit{appearing} to grasp, not actually grasping!
        \end{itemize}
...
\begin{frame}{When Inner Misalignment Becomes Apparent}
    \begin{block}{Role of Distributional Shift}
        Misalignment can go undetected during training if the proxy goal works well. It becomes apparent when facing different environments.
    \end{block}
    ...
    \small
    Distributional shift \textit{reveals} the problem—it doesn't cause it. The model was pursuing an unintended goal all along.
\end{frame}
